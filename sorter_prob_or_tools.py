...
process_cost=pd.read_csv('process_cost.csv')
dis_input=pd.read_csv('dist_input.csv')

dis_input=dis_input.set_index(['oc','cn'])

max_check=9

### 24ft vehicle type details ###

ship_per_bag=11
bags_per_veh=600
ship_per_veh=ship_per_bag*bags_per_veh
...
pcos_dict={}
for i in range(process_cost.shape[0]):
    pcos_dict[process_cost['center'][i]]=process_cost['cost_per_ship'][i]
...
Q = []
for i in range(len(ori)):
    Q.append(model2.NumVar(0, infinity,'q_' + ori[i]))  ###########Change infinity
    
truck = []
for k in range(len(ori)):
    truck.append([])
    for m in range(len(des)):
        truck[k].append(model2.NumVar(0, infinity,'truck_' + ori[k] + ' -> ' + des[m]))
...
for i in ori:
    ...
    for j in range(len(des)):
        exp3 += exp[k][j]
    model2.Add(Q[k] == exp3, "processed_vol" + i)

for i in range(len(ori)):
    for j in range(len(des)):
        model2.Add(truck[i][j]>=exp[i][j]/ship_per_veh, "truck_cons" + ori[i] + ',' + des[j] + ')')

process_cost=0
for i in enumerate(ori):
    process_cost+=Q[i[0]]*pcos_dict[i[1]]

truck_cost=0
for i in enumerate(ori):
    for j in enumerate(des):
        truck_cost+=truck[i[0]][j[0]]*(dis_input[i,j]['dist']*variable_cost)

final_cost=truck_cost+process_cost 
